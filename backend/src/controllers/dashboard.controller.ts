...
export const getDashboardMetrics = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (role === RoleId.lc) {
      ...
      // Fetch task stats
      const tasks = await prisma.check.findMany({ where: { branchId } });
      ...
      const openComplaintsCount = await prisma.complaint.count({
        where: { branchId, status: { notIn: [ComplaintStatus.RESOLVED] } }
      });
      ...
      const activeTasks = await prisma.check.findMany({
      ...
      complaints
        .filter(c => 0 > 20000 && c.status !== ComplaintStatus.RESOLVED)
        .forEach(c => {
      ...
      complaints
        .filter(c => c.status === ComplaintStatus.VENDOR_PENDING)
        .forEach(c => {
          const br = branches.find(b => b.id === c.branchId);
