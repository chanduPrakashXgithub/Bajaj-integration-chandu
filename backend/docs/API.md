...
Mark daily attendance. Also auto-generates pending checks from the LC's weekly plan.
...
Get the authenticated user's attendance calendar with weekly checks.
...
Mark a check as completed (no proof upload).
...
Submit a check with photo proof. `multipart/form-data`.
...
| `notes` | string | Optional text remarks appended to check notes. |

**Response** `200`
```json
{
  "message": "Proof submitted successfully",
  "check": { "...updated check with proofUrl..." }
}
```

### POST /api/tasks/:id/revoke

Revoke or reopen a check.
...
Soft-archive a check (BM only).

**Response** `200`
```json
{
  "check": { "...archived check..." }
}
```
...
Returns branch KPIs, checks, complaints, appliances, and today's attendance in a single call.

#### GET /api/lc/tasks?status=
Checks visible to this LC (their branch, assigned to them or audience=lc).

#### GET /api/lc/attendance/calendar?month=&year=
LC's personal attendance calendar + completed checks.

#### POST /api/lc/attendance
Mark attendance + submit weekly check plan (same as POST /api/attendance).
...
Scoped attendance roster + staff users + today's checks.

#### GET /api/bm/tasks
Checks for branches in BM scope.
...
All attendance + all users + checks.
...
#### GET /api/rm/tasks
All checks (RM scope) with lean fields.
