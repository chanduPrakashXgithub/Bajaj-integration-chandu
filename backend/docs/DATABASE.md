...
- `createdChecks` → Check[] (via assignedById)
- `assignedChecks` → Check[] (via assignedToId)
- `completedChecks` → Check[] (via completedById)
...
| notes | Text | - | Check notes/description |
...
Check plan items embedded within attendance logs.
...
User ──branch──▶ Branch
User ◀──assignedTo── Check
User ◀──assignedBy── Check
User ◀──completedBy── Check
...
| Check | `branchId`, `assignedToId`, `status` |
