...
2. **Scope Controls**: Regional Managers (RM) retain ownership of global configuration mutations (like staff shifts and budget caps). Branch Managers (BM) oversee operational check schedules, audits, and approvals.
...
- **MVP Excluded / Deferred**:
  - `PATCH /attendance/:id/tasks` (Weekly check plan edit) — *Weekly check plan must not be editable after clock-in.*
...
- **Role**: View weekly checklists and execute check submissions.
- **MVP Excluded / Deferred**:
  - `POST /tasks/:id/delegate` (Check delegation) — *Out of scope for MVP.*
  - `PATCH /tasks/:id/defer` (Check deadline snooze) — *Out of scope for MVP.*
...
### 2. `BranchManagerMonitoringScreen.tsx` (Check Manager)
- **Role**: Schedules checks and monitors staff checklist completions.
- **MVP Approved APIs**:
  - **Edit Check (`PUT`)**:
    - **Endpoint**: `PUT /api/tasks/:id`
    - **Description**: Allows editing of check details, checklist metrics, or assignee.
  - **Archive Check (`PATCH` - Status Change)**:
    - **Endpoint**: `PATCH /api/tasks/:id/archive`
    - **Description**: Sets check status to `Archived` instead of deleting.
...
- **Role**: Profile details, historical check records, and documents.
- **MVP Excluded / Deferred**:
