...
### 2. GET /lc/tasks
Returns checks scoped to the LC's assigned branch.

- **URL**: `/api/lc/tasks`
- **Method**: `GET`
- **Query Params**:
  - `status` (string, optional): filter by check status (`Pending`, `InProgress`, `Completed`, `Revoked`).
- **Response**: `200 OK`
- **Payload**:
```json
{
...
Submit check-in for the day and enter weekly check plan.
...
### 2. GET /bm/attendance
Returns scoped attendance logs (limited to last 500), users lists, and checks in scope.
...
### 3. GET /bm/tasks
Returns checks for branches in the BM's scope.
