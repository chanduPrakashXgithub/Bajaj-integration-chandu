...
| `rm` | Regional Manager | System-wide scope. | Has global access to all branches, users, checks, complaints, and financial data. |
...
### 2. AttendanceLog & Weekly Checks Plan
- Each `AttendanceLog` represents a user's clock-in event for a specific date.
- LCs submit weekly checks in their check-in flow. These are stored in `WeeklyTaskPlanItem` with a relation to the corresponding `AttendanceLog`.
- Scoping rule:
...
model AttendanceLog {
  ...
  weeklyTasks  WeeklyTaskPlanItem[] // Scoped check items submitted weekly
}
