...
| `checks` | `/lc/tasks` | `/bm/tasks` | `/rm/tasks` |
...
if (endpoint === "/lc/dashboard") {
  // Payload: { branch, checks, complaints, appliances, todayAttendance }
  const { branch, checks, complaints, appliances, todayAttendance } = res.data;
  
  if (branch) setBranches([branch]);
  if (checks) setScopedTasks(checks);
